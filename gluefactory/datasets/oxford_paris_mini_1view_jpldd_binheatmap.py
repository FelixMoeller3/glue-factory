...
def plot_predictions(pred, data):
    kp_gt = data["gt_keypoints"][0].cpu().numpy()
    hm_gt = data["deeplsd_line_heatmap"][0].cpu().numpy()
    img = data["image"][0].permute(1, 2, 0).cpu().numpy()

    kp_pred = pred["keypoints"][0].cpu().numpy()
    hm_pred = pred["line_binary_heatmap"][0].cpu().numpy()
    ...
    return {"KP": fig_pred_kp, "HeatMap": fig_pred_hm}



class OxfordParisMiniOneViewJPLDD_BinHeatmap(BaseDataset):
    """
...
class _Dataset(torch.utils.data.Dataset):
    def __init__(self, conf, image_sub_paths: list[str], split, augmentation):
        ...
        if conf.load_features.do:
            ...
            for img_path in self.image_sub_paths:
                ...
                bin_heatmap_file = img_folder / "line_binary_heatmap.jpg"
                ...
                if keypoint_file.exists() and bin_heatmap_file.exists():
    ...
    def _read_groundtruth(self, image_folder_path, shape: int = None) -> dict:
        ...
        reshape_scales = None
        reshaped_size = None
        pt_heatmap_gt_key_name = self.conf.load_features.point_gt.data_keys[0]
        ...
        line_hm_gt_key = self.conf.load_features.line_gt.data_keys[0]
        ...
        # Load Line Binary Heatmap
        line_heatmap = read_image(image_folder_path / "line_binary_heatmap.jpg", True)
        line_bin_heatmap = line_heatmap.astype(np.float32) / 255.0  # normalize to {0,1}

        line_bin_heatmap = torch.from_numpy(line_bin_heatmap).to(dtype=torch.float32)

        original_size = line_bin_heatmap.shape
        # reshape heatmap if needed
        if shape is not None:
            preprocessor_hm_out = self.preprocessors[shape](line_bin_heatmap.unsqueeze(0))
            reshape_scales = preprocessor_hm_out["scales"]
            reshaped_size = preprocessor_hm_out["image_size"]
            line_bin_heatmap = preprocessor_hm_out["image"].squeeze(
                0
            )  # only store image here as padding map will be stored by preprocessing image
        features[line_hm_gt_key] = line_bin_heatmap
        ...
        if len(keypoints) < self.max_num_gt_kp:
            # pad with zeros if less keypoints than max
            keypoints = torch.cat(
                [
                    keypoints,
                    torch.zeros(
                        (self.max_num_gt_kp - len(keypoints), 2), dtype=torch.float32
                    ),
                ]
            )
            kps = torch.cat(
                [
                    kps,
                    torch.zeros(self.max_num_gt_kp - len(kps), dtype=torch.float32),
                ]
            )
        
        # create heatmap
        heatmap = np.zeros_like(line_bin_heatmap)
        ...
        features[pt_heatmap_gt_key_name] = heatmap
