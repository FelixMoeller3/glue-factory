...
class JointPointLineDetectorDescriptor(BaseModel):
    ...
    def _init(self, conf) -> None:
        ...
        # set loss fn for heatmap
        assert self.conf.training.loss.hm_loss_name in [
            "focal_loss",
        ]
        if self.conf.training.loss.hm_loss_name == "focal_loss":
            self.loss_fn_hm = self.focal_loss_hm
    ...
    def _forward(self, data: dict) -> torch.Tensor:
        ...
        line_binary_heatmap = self.distance_field_branch(feature_map)
        # remove additional dimensions of size 1 if not having batchsize one
        line_binary_heatmap = (
            line_binary_heatmap.squeeze(1)
            if line_binary_heatmap.shape[0] == 1
            else line_binary_heatmap.squeeze()
        )
        if self.conf.timeit:
            self.timings["line-df"].append(sync_and_time() - start_line_df)
        output["line_binary_heatmap"] = line_binary_heatmap
        ...
        if self.conf.line_detection.do and not self.training:
            ...
            if output.get("line_anglefield", None) is None:
                ...
                line_angle_field = torch.zeros_like(line_binary_heatmap)

            for df, af, kp, desc in zip(
                line_binary_heatmap, line_angle_field, rescaled_kp, keypoint_descriptors
            ):
                """
                "line_detection": {  # by default we use the POLD2 Line Extractor (MLP with Angle Field)
                    "do": True,
                ...
                valid_lines.append(
                    torch.ones(len(lines[-1])).to(line_binary_heatmap[-1].device)
                )
    ...
    def focal_loss_hm(self, prediction: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        alpha = self.conf.training.loss.hm_loss_parameters.focal_alpha
        gamma = self.conf.training.loss.hm_loss_parameters.focal_gamma
        epsilon = 1e-6  # Small value to avoid log(0)
    ...
    def loss(self, pred: dict, data:dict) -> dict:
        ...
        gt_mask = data["deeplsd_line_heatmap"]
        line_df_loss = self.loss_fn_hm(
            pred["line_binary_heatmap"] * padding_mask,
            gt_mask * padding_mask,
        ).mean(dim=(1, 2))
        losses["line_binary_heatmap"] = line_df_loss
